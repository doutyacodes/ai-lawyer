...
import KnowYourRightsUI from './_components/KnowYourRightsUI';
...
export default function LegalAssistantPage() {
  ...
  const [selectedOption, setSelectedOption] = useState(''); // 'emergency', 'non-emergency', 'know-rights'
  const [formData, setFormData] = useState({
    reportingFor: '', // 'myself' or 'someone_else'
    problem: '',
    optionType: '' // Will store the selected option
  });
  ...
  useEffect(() => {
    ...
    const resultType = formStorage.load('results_type');
    if (savedResults && resultType) {
  ...
  const validateForm = () => {
    ...
    if (!formData.reportingFor) {
      newErrors.reportingFor = 'Please specify who this is for';
    }
    
    if (!formData.problem.trim()) {
      newErrors.problem = 'Please describe the situation in detail';
    } else if (formData.problem.trim().length < 20) {
      newErrors.problem = 'Please provide more details about the situation';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to get legal guidance');
      }
      console.log("consoe",result);
      
      setResults(result.data);
      setResponseType(result.responseType); // Add this line
      setCurrentStep('results');

      // Save results to storage
      formStorage.save('results_data', result.data);
      formStorage.save('results_type', result.responseType);
      formStorage.save('form_data', formData);
    } catch (error) {
      console.errolog('API Error:', error);
  ...
  const handleSaveResults = async () => {
    ...
    const response = await fetch('/api/save-query', {
      ...
      if (data.success) {
        ...
        formStorage.clear('results_type');
  ...
  if (currentStep === 'selection') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 flex items-center justify-center px-4 py-8">
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 sm:p-8 md:p-12 border border-white/20 shadow-2xl max-w-3xl w-full">
          ...
          <div className="space-y-4 sm:space-y-6">
            <button
              onClick={() => handleOptionSelection('emergency')}
            ...
            >
              <span className="flex items-center justify-center px-4">
                ...
                <div className="text-left">
                  <div className="font-bold text-base sm:text-lg">Emergency Legal Help</div>
                  <div className="text-xs sm:text-sm text-red-100 mt-1">
                    Immediate danger, arrest, injury, or urgent legal matters
                  </div>
                </div>
              </span>
            </button>
            ...
            <button
              onClick={() => handleOptionSelection('non-emergency')}
              className="w-full py-4 sm:py-6 text-base sm:text-lg font-semibold text-white bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl hover:from-blue-400 hover:to-blue-500 transform hover:scale-105 transition-all duration-300 shadow-lg border-2 border-blue-400/50"
            >
              <span className="flex items-center justify-center px-4">
                <span className="mr-3 text-2xl sm:text-3xl">📋</span>
                <div className="text-left">
                  <div className="font-bold text-base sm:text-lg">General Legal Consultation</div>
                  <div className="text-xs sm:text-sm text-blue-100 mt-1">
                    Legal advice, document help, or general guidance
                  </div>
                </div>
              </span>
            </button>
            ...
            <button
              onClick={() => handleOptionSelection('know-rights')}
              className="w-full py-4 sm:py-6 text-base sm:text-lg font-semibold text-white bg-gradient-to-r from-green-500 to-green-600 rounded-xl hover:from-green-400 hover:to-green-500 transform hover:scale-105 transition-all duration-300 shadow-lg border-2 border-green-400/50"
            >
              <span className="flex items-center justify-center px-4">
                <span className="mr-3 text-2xl sm:text-3xl">🛡️</span>
                <div className="text-left">
                  <div className="font-bold text-base sm:text-lg">Know My Rights</div>
                  <div className="text-xs sm:text-sm text-green-100 mt-1">
                    Understand your legal rights in specific situations
                  </div>
                </div>
              </span>
            </button>
          ...
          <div className="mt-8 sm:mt-12 text-center">
            ...
            <p className="text-xs text-gray-400 mt-4 sm:mt-6">
              This provides general legal information only. For complex matters, consult a qualified attorney.
            </p>
          </div>
        </div>
      </div>
  ...
  // Form Screen (same for all three options)
  if (currentStep === 'form') {
    const getFormConfig = () => {
      switch (selectedOption) {
        ...
        case 'non-emergency':
          return {
            ...
            subtitle: 'Tell us about your legal situation',
            icon: '📋',
            color: 'blue',
            placeholder: 'Please describe your legal situation in detail. Include:\n\n• What is the legal issue or question?\n• When did this situation arise?\n• Where did it happen?\n• Who else is involved?\n• What documents or evidence do you have?\n• What outcome are you hoping for?\n• Have you taken any legal steps already?\n• Any other relevant information\n\nThe more details you provide, the better guidance we can offer.',
        ...
        case 'know-rights':
          return {
            ...
            subtitle: 'Help us understand your situation to explain your rights',
            icon: '🛡️',
            color: 'green',
            placeholder: 'Please describe the situation where you want to know your rights. Include:\n\n• What specific situation are you in?\n• Where is this happening? (country, state, city)\n• Who is involved? (police, employer, landlord, etc.)\n• What actions have been taken so far?\n• What are you being asked to do?\n• Are you being threatened or pressured?\n• What specific rights are you concerned about?\n• Any documents or notices you\'ve received\n\nProvide as much context as possible so we can explain your rights clearly.',
            alertBg: 'from-green-50 to-emerald-50',
            alertBorder: 'border-green-200',
            alertText: 'text-green-700',
            alertTitle: 'text-green-800',
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Form Container */}
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/50 p-6 md:p-12">
            <form onSubmit={handleSubmit} className="p-4 lg:p-12">
              {/* Priority Alert */}
              {selectedOption === 'emergency' && (
                <div className={`mb-8 bg-gradient-to-r ${config.alertBg} border-2 ${config.alertBorder} rounded-2xl p-6`}>
              ...
              <div className="mb-8">
                <label className="block text-lg font-semibold text-gray-700 mb-4">
                  {formData.reportingFor === 'someone_else' ? 'Describe their situation *' : 'Describe your situation *'}
                </label>
                <textarea
                  value={formData.problem}
                  onChange={(e) => handleInputChange('problem', e.target.value)}
                  className={`w-full px-6 py-4 bg-white border-2 rounded-2xl text-gray-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-300 resize-none ${
                ...
                <p className="text-gray-500 text-sm mt-2">
                  💡 Tip: The more details you provide, the better and more specific guidance we can offer.
                </p>
              </div>
...
console.log("currentstep",currentStep, "result", results, "responseType",responseType)
...
if (currentStep === 'results' && results){
    ...
    <div className="container mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div className="max-w-6xl mx-auto">
        ...
        {responseType === 'know_rights_response' ? (
          <KnowYourRightsUI results={results} />
        ) : (
          <div className="space-y-4 sm:space-y-6">
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
              <div className="container mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
  ...
  return (
    <div>
      {loading && <LoadingOverlay message="Analyzing your situation..." />}
    </div>
  );
}